...
```{r ans1}
# logicBig
logicbig<-as.integer(x) > 50  #要先把character換成數字進行核對!!文字不能跟數字比對
logicbig
```
...
```{r ans52}
# westYears
year<-str_sub(twDates2,3,5)
year<-as.integer(year)
years<-as.character(year+1911)
westYears<-str_c("西元",years,"年",str_sub(twDates2,7,8))
westYears
```
...
str_c(as.character(str_sub(westYears,3,6)),monthVector,"01",sep = "-") -> westDates # 貼成YYYY-MM-DD 字串格式
westDates
